...
{
    // 此处编写用户代码
    static uint16 control_num = 0, turn_control_num = 0, speed_control_num = 0;
    turn_control_num++;
    control_num++;
    speed_control_num++;
        // gyro_calculate();
    ...
    // 角速度环输出(5ms)
    if (turn_control_num >= 1) {
        turn_control_num = 0;
        Get_Speed(); // 获取速度 5ms speedL,speedR
        Avg_speed = data_filtering(filter_speed, (encoder_1 + encoder_2) / 2.0, 5);
        if (Fabs(g_z_degree) > 30)                                     // 如果角速度大于30°每秒
            Correct_speed = Avg_speed + 0.095 * Fabs(g_z_degree) * 65; //? 修正速度
        else
            Correct_speed = Avg_speed;
        EncoderCount(); // 计算行驶距离
                        // if (start_flag && start_num >= 500) {
                        // if (g_TrackType.m_u8GarageFlag == GARAGE_STOP) {
        // static uint8 num = 0; // 多次符合条件
        // now_speed        = -10;
        // if (Kabs(speedL) < 8 && Kabs(speedR) < 8) {
        //     num++;
        //     if (num >= 2) {
        //         Speed_ZERO_Control(0, 0);
        //         set_speed((int)ZERO_outL, (int)ZERO_outR);
        //         if (num >= 5) {
        //             stop_flag  = 1;
        //             start_flag = 0;
        //             stop_init  = 0;
        ...
        // if (charge_speed_stop) // 充电停车
        // {
        //     enconder_ZERO_Control();
        // } else // 非充电停车执行
        // {
